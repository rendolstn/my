              ...
              <?php
                ...
                if(isset($_GET['do'])){
                  ...
                  if(isset($_GET['catID'])){
                    $id = $_GET['catID'];
                    $abc = $_GET['abc'];
                    $sql_jeniss = mysqli_query($conn,"SELECT * FROM `tb_jenis` WHERE cuid = '$abc'") or die(mysqli_error());
                    ...
                    else if($jenisnyaa == 'Social'){
                        $tables = 'tb_produk_social';
                    }
                    else if($jenisnyaa == 'Jasa Joki'){
                        $tables = 'tb_produk';
                    }
                    else if($jenisnyaa == 'Pascabayar'){
                        $tables = 'tb_pascabayar';
                    }
                    else if($jenisnyaa == 'Kartu Hadiah'){
                        $tables = 'tb_produk';
                    }
                    $sql_2 = mysqli_query($conn,"SELECT * FROM $tables WHERE cuid = '$id'") or die(mysqli_error());
                  ...
                  <form role="form" class="form-group" action="<?php echo $urlweb; ?>/function/add-produkmanual.php" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group mb-2">
                      ...
                      <select name="product_type" id="jenis" class="form-control" required>
                        ...
                        ?>
                        <option value="<?php echo $sj['cuid']; ?>"<?php if(isset($_GET['catID'])) { if($s2['product_type'] == $sj['cuid']) { echo 'selected = selected'; }} ?>> <?php echo $sj['jenis']; ?></option>
                        <?php } ?>
                      </select>
                    </div>
                    <div class="form-group mb-2">
                    ...
                    <?php } else { ?>
                    <input class="form-control" type="hidden" name="product_type" value="<?php if(isset($_GET['catID'])) { echo $abc; } ?>">
                    <?php } ?>
                    <div class="form-group mb-2">
              ...
              <div class="card">
                <div class="card-header py-2">
                  <ul class="nav nav-pills card-header-pills" role="tablist">
                    <?php
                      $sql_jenis = mysqli_query($conn,"SELECT * FROM `tb_jenis` WHERE `status` = 1 AND `cuid` NOT IN(5,6,7) ORDER BY cuid ASC") or die(mysqli_error());
                    ...
                    ?>
                    <li class="nav-item">
                      <button type="button" class="nav-link<?php if($no == 1) { echo ' active'; } ?>" role="tab" data-bs-toggle="tab" data-bs-target="#<?php echo str_replace(' ','',strtolower($sj['jenis'])); ?>" aria-controls="<?php echo str_replace(' ','',strtolower($sj['jenis'])); ?>" aria-selected="true"> <?php echo $sj['jenis']; ?> </button>
                    </li>
                    <?php } ?>
                  </ul>
                </div>
                  ...
                  <div class="tab-content p-0">
                    <?php
                        ...
                        while($sj = mysqli_fetch_array($sql_jenis)){
                          ...
                          else if($jenisnya == 'Kartu Hadiah'){
                            $parent = 8;
                            $table = 'tb_produk';
                          }
                    ?>
                    <div class="tab-pane fade<?php if($nos == 1) { echo ' show active'; } ?>" id="<?php echo str_replace(' ','',strtolower($sj['jenis'])); ?>" role="tabpanel">
                      <div class="card-datatable table-responsive">
                        <table class="invoice-list-table table border-top default-datatable">
                          <thead>
                            <tr class="bg-info">
                          ...
                          <tbody>
                            <?php
                              $sql_1 = mysqli_query($conn,"SELECT * FROM $table WHERE jenis = 10 AND product_type = '$parent' ORDER BY cuid ASC") or die(mysqli_error($conn));
                              $no=0;
                            ...
                            <tr>
                              ...
                              <td class="text-center" style="vertical-align: middle; white-space: normal; font-size: 14px; width: 10%;">
                                <a href="<?php echo $urlweb; ?>/product_manual/?do=add&catID=<?php echo $s1['cuid']; ?>&abc=<?php echo $s1['product_type']; ?>" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i></a>
                                <a href="<?php echo $urlweb; ?>/function/del-produkmanual.php?cuid=<?php echo $s1['cuid']; ?>&abc=<?php echo $s1['product_type']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure want remove this data?');"><i class="fa fa-trash"></i></a>
                              </td>
                            </tr>
                            <?php } ?>
                          </tbody>
