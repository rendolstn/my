                  ...
                  <div class="card">
                    <div class="card-body">
                      <?php
                        ...
                        if(isset($_GET['catID'])){
                          $catID = $_GET['catID'];
                          $sql_2 = mysqli_query($conn,"SELECT * FROM `tb_reward` WHERE cuid = '$catID'") or die(mysqli_error());
                      ...
                      <form role="form" action="<?php echo $urlweb; ?>/function/add-reward.php" method="post">
                        ...
                        <div class="form-group mb-2">
                          ...
                          <input type="number" name="min_order" value="<?php if(isset($_GET['catID'])) { echo $s2['min_order']; } ?>" class="form-control" required>
                        </div>
                        <div class="form-group mb-2">
                          <label class="form-label">Quota</label>
                          <input type="number" name="kuota" value="<?php if(isset($_GET['catID'])) { echo $s2['kuota']; } ?>" class="form-control" required>
                        </div>
                        <div class="form-group mb-2">
                      ...
                      <table id="default-datatable" class="invoice-list-table table border-top">
                        <thead>
                          <tr class="bg-info">
                            ...
                            <th class="text-center" style="vertical-align: middle;">Quota</th>
                            <th class="text-center" style="vertical-align: middle;">Min Order</th>
                        ...
                        <tbody>
                          <?php
                            error_reporting(0);
                            $sql_1 = mysqli_query($conn,"SELECT * FROM `tb_reward` ORDER BY cuid DESC") or die(mysqli_error());
                            $no=0;
                          ...
                          <tr>
                            ...
                            <td class="text-left" style="vertical-align: middle; white-space: normal;"><?php echo $s1['title']; ?><br><small><?php if($s1['status'] == 1){ echo 'Not Available'; } else { echo 'Available'; } ?></small></td>
                            <td class="text-right" style="vertical-align: middle; white-space: normal; text-align: right;"><?php echo number_format($s1['nominal']); ?> <?php if($s1['satuan'] == 0){ echo '%'; } else { echo 'IDR'; } ?></td>
                            <td class="text-center" style="vertical-align: middle; white-space: normal;"><?php echo number_format($s1['kuota']); ?></td>
                            <td class="text-right" style="vertical-align: middle; white-space: normal; text-align: right;"><?php echo number_format($s1['min_order']); ?></td>
                            <td class="text-left" style="vertical-align: middle; white-space: normal; font-size: 14px;">
                                <ol>
                                    <?php
                                        $sql_2 = mysqli_query($conn,"SELECT * FROM `tb_reward_produk` WHERE title = '$titlenya' ORDER BY cuid ASC") or die(mysqli_error());
                            ...
                            <td class="text-center" style="vertical-align: middle; font-size: 14px;">
                              <a href="<?php echo $urlweb; ?>/function/del-reward.php?cuid=<?php echo $s1['cuid']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure want remove this data?');"><i class="fa fa-trash"></i></a>
                            </td>
                          </tr>
                          <?php } ?>
                        </tbody>
